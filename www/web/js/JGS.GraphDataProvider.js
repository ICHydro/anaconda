(function (JGS, $, undefined) {
  ...
  */
  JGS.GraphDataProvider.prototype.loadData = function (seriesName, sensor, rangeStartDateTm,
     rangeEndDateTm, detailStartDateTm, detailEndDateTm, pixelWidth, chartType) {
    ...
    this.first = (rangeStartDateTm && detailStartDateTm);
    var includeMinMax = undefined;
    ...
    if (rangeStartDateTm && rangeEndDateTm) {
      ...
      includeMinMax = (moment.utc(rangeEndDateTm).unix() - moment.utc(rangeStartDateTm).unix() > 24*3600*31);
    ...
    if (detailStartDateTm && detailEndDateTm) {
      ...
      includeMinMax = (moment.utc(detailEndDateTm).unix() - moment.utc(detailStartDateTm).unix() > 24*3600*31);
      ...
      if (this.first && len > 0) {
        ...
        for (var i = 0; i < splicedData.length; i++) {
          if (dataLoadReq.includeMinMax)
            ...
            dyData.push([new Date(splicedData[i].x), splicedData[i].avg]);
        }

        graphData = {
          includeMinMax: dataLoadReq.includeMinMax,
          chartType: dataLoadReq.chartType,
          dyData: dyData,
  ...
  JGS.GraphDataProvider.prototype._spliceRangeAndDetail = function (rangeDps, detailDps) {
    ...
    if (rangeDps.length == 0 && detailDps.length == 0) {
      ...
      for (var i = startEndRangeIdx; i >= lastRangeIdx; i--) {
        if (i <= 1 || new Date(rangeDps[i - 1].x) <= detailEndX) {
          break;
        } else {
          startEndRangeIdx--;
        }
