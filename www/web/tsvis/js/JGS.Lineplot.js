(function (JGS, $, undefined) {    // WB: wrapping in () means that entire block is essentially an anonymous function executed directly with arguments at the very end
  ...
  JGS.Lineplot = function (pageCfg) {
    console.log("Lineplot object created");
  ...
  JGS.Lineplot.prototype.init = function () {
    console.log("Lineplot init");
    this.showSpinner(true);
    this._setupRangeButtons();
//    this.setUpRefresh();

    // get the sensor name:
    var sensor = $("#select_sensor").val();
    console.log(sensor);
  ...
  /**
   * Method for the refresh button. This is essentially the same as the initialization process above.
   *
   * @method
   *

  JGS.Lineplot.prototype.setUpRefresh = function () {
    var self = this;
 ...
 /**
  ...
  JGS.Lineplot.prototype._loadNewDetailData = function () {
    ...
    this.graphDataProvider.loadData("temperature", sensor, null, null, this.detailStartDateTm, this.detailEndDateTm, this.$graphCont.width());
  };
  ...
  JGS.Lineplot.prototype.drawDygraph = function (graphData) {
    console.log("drawing dygraph");
    var dyData = graphData.dyData;

    if (dyData.length == 0) { // need Comment
      //dyData = "X\n";
...
//    this.detailEndDateTm = axisXEndDateTm;
  ...
  JGS.Lineplot.prototype._onDyZoomCallback = function (minDate, maxDate, yRanges) {
    ...
    if (doDataLoad === true) {
      console.log("zoom call back load detail data");
      this._loadNewDetailData();
    }
